...
function getNextFriday(date) {
  ...
  let daysToAdd = 5 - currentDate.getDay();
  if (daysToAdd <= 0) {
    daysToAdd += 7;
  }

  const nextFriday = new Date(currentDate);
  nextFriday.setDate(currentDate.getDate() + daysToAdd);
...
function getWeekNumberByDate(date) {
  ...
  const januaryFourth = new Date(targetDate.getFullYear(), 0, 4);
  const dayDifference = (targetDate - januaryFourth) / 86400000;
  const weekNumber = 1 + Math.ceil(dayDifference / 7);
  return weekNumber;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (currentDate <= new Date(endYear, endMonth - 1, endDay)) {
    for (let i = 0; i < countWorkDays; i += 1) {
      if (currentDate <= new Date(endYear, endMonth - 1, endDay)) {
        dates.push(
